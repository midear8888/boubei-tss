 ...
 *      将匿名用户和普通用户完全一样处理
 * 
 * 系统重启后，所用用户登录状态都将失效，因Filter0（地址白名单）在，/auth/无法自动根据在线用户库完成自动登录。
 * Global里在每次系统重启后，清空在线用户表。 
 * </pre>
 *
...
public class Filter4AutoLogin implements Filter {
	...
	/**
	 * <pre>
	 * 验证用户是否合法，包括正常登录、通过在线用户库单点登录（系统被重启）、匿名用户登录等的身份验证。 
	 * 1、如果身份合法，返回身份证对象； 
	 ...
	 */
	static IdentityCard validate(String userIdentifierClassName) throws UserIdentificationException {
