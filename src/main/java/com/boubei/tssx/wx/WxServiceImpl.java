...
public class WxServiceImpl implements WxService {
	...
	@SuppressWarnings("unchecked")
	public String sendWxGZHMsgAll(Map<String, String> requestMap) throws IOException {
		...
		String mobile = requestMap.get("phone");
		String appid = requestMap.get("appid");
		String hql = "from WxGZHBindPhone where mobile in (" + mobile + ") and appid = '" + appid + "'";
		List<WxGZHBindPhone> bindPhones = (List<WxGZHBindPhone>) commonDao.getEntities(hql);
		...
		if(bindPhones.size() == 0) {
			...
			for(WxGZHBindPhone bindPhone : bindPhones) {
				arr.add(bindPhone.getOpenid());
			}
			json.put("touser", arr);
		}
		json.put("template_id", requestMap.get("template_id"));
...
//		if(bindPhones.size() == 0) {
//			json.put("touser", requestMap.get("unionid")); // 适用于小程序与公众号拥有共同unionid的情况
//			str = sendBase(json, postMethod);
//		} else {
//			for(WxGZHBindPhone bindPhone : bindPhones) {
//				json.put("touser", bindPhone.getOpenid());
//				str += sendBase(json, postMethod);
//			}
//		}
//		
...
//	@SuppressWarnings("unchecked")
//	private String sendBase(JSONObject json, PostMethod postMethod) throws IOException{
//		String requestData = json.toString();
//		byte[] b = requestData.getBytes("UTF-8");
//        InputStream is = new ByteArrayInputStream(b, 0, b.length);
//        RequestEntity re = new InputStreamRequestEntity(is, b.length, "application/json; charset=UTF-8");
